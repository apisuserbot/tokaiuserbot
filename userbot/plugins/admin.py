...
PP_ERROR = "`Gagal Proses Gambar!!!`"
NO_ADMIN = "`Bos bukan admin!!!`"
NO_PERM = "`Bos tidak punya permission itu!! Kasihan...`"
CHAT_PP_CHANGED = "`Foto grup diganti!!!`"
INVALID_MEDIA = "`Media invalid!!!`"
...
async def set_group_photo(gpic):
    ...
    if not gpic.is_group:
        await edit_or_reply(gpic, "`Maaf, ini bukan grup!!!`")
        return
    ...
    if photo:
        try:
            ...
            await edit_or_reply(gpic, f"**Gagal : **`{str(e)}`")
        if BOTLOG and sandy:
...
async def promote(promt):
    ...
    if not promt.is_group:
        await edit_or_reply(promt, "`Maaf, ini bukan grup!!!`")
        return
    ...
    try:
        ...
        await catevent.edit("`Promoting sukses!!! Ciee admin baru...`")
...
async def demote(dmod):
    ...
    if not dmod.is_group:
        await edit_or_reply(dmod, "`Maaf, ini bukan grup!!!`")
        return
    ...
    try:
        ...
        await catevent.edit(NO_PERM)
        return
    await catevent.edit("`Demoted sukses!!! Kasihan...`")
    if BOTLOG:
...
async def ban(bon):
    if bon.fwd_from:
        return
    if not bon.is_group:
        await edit_or_reply(bon, "`Maaf, ini bukan grup!!!`")
        return
    ...
    if not admin and not creator:
        ...
        return
    catevent = await edit_or_reply(bon, "`Selamat tinggal...`")
    ...
    try:
        ...
        await catevent.edit(
            "`Aku tidak punya alasan... Tapi dia tetap terbanned!!`"
        )
        return
    if reason:
        await catevent.edit(
            f"{_format.mentionuser(user.first_name ,user.id)}` Telah dibanned!!`\n**Karena : **`{reason}`"
        )
    else:
        await catevent.edit(
            f"{_format.mentionuser(user.first_name ,user.id)} `Telah terbanned!!`"
        )
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    if not unbon.is_group:
        await edit_or_reply(unbon, "`Maaf, ini bukan grup!!!`")
        return
    ...
    try:
        ...
        await catevent.edit(
            f"{_format.mentionuser(user.first_name ,user.id)} `Unbaned sukses!! Jangan nakal lagi ya...`"
        )
        if BOTLOG:
        ...
        await catevent.edit("`Error 404 Not Found`")
...
async def startmute(event):
    if event.fwd_from:
        return
    if event.is_private:
        await event.edit("`Error 404 Not Found`")
        await sleep(2)
        ...
        if is_muted(userid, chat_id):
            return await event.edit(
                "`User telah dimute!!! Gimana sih!?`"
            )
        try:
            mute(userid, chat_id)
        except Exception as e:
            await event.edit(f"**Gagal **\n`{str(e)}`")
        else:
            await event.edit("`User dimute!!!\n`")
        if BOTLOG:
        ...
        if not user:
            return
        if user.id == bot.uid:
            return await edit_or_reply(event, "`Maaf, aku tidak bisa mute penciptaku!!!`")
        if is_muted(user.id, event.chat_id):
            return await edit_or_reply(
                event, "`User telah dimute!!! Gimana sih!?`"
            )
        try:
            admin = chat.admin_rights
            creator = chat.creator
            if not admin and not creator:
                await edit_or_reply(
                    event, "`Permission tidak lengkap!! kasihan...` ಥ﹏ಥ  "
                )
                return
            ...
            try:
                if result.participant.banned_rights.send_messages:
                    return await edit_or_reply(
                        event,
                        "`User telah dimute!!! gimana sih bos!?`",
                    )
            except Exception as e:
                LOGS.info(str(e))
            ...
            if "admin_rights" in vars(chat) and vars(chat)["admin_rights"] is not None:
                if chat.admin_rights.delete_messages is not True:
                    return await edit_or_reply(
                        event,
                        "`Permission tidak lengkap!! Kasihan... ಥ﹏ಥ`",
                    )
            elif "creator" not in vars(chat):
                return await edit_or_reply(
                    event, "`Permission tidak lengap!! Kasihan...` ಥ﹏ಥ  "
                )
            try:
                mute(user.id, event.chat_id)
            except Exception as e:
                return await edit_or_reply(event, f"**Gagal **\n`{str(e)}`")
        except Exception as e:
            return await edit_or_reply(event, f"**Gagal : **`{str(e)}`")
        if reason:
            await edit_or_reply(
                event,
                f"{_format.mentionuser(user.first_name ,user.id)} `telah dimute di {event.chat.title}`\n"
                f"`Alasan:`{reason}",
            )
        else:
            await edit_or_reply(
...
async def endmute(event):
    if event.fwd_from:
        return
    if event.is_private:
        await event.edit("`404 Error Not Found`")
        await sleep(1)
        userid = event.chat_id
        replied_user = await event.client(GetFullUserRequest(userid))
        chat_id = event.chat_id
        if not is_muted(userid, chat_id):
            return await event.edit(
                "`User sedang tidak dimute disini\n`"
            )
        try:
        ...
        try:
            if is_muted(user.id, event.chat_id):
                ...
                try:
                    ...
                    return await edit_or_reply(
                        event,
                        "`User ini tidak dibisukan lagi, enjoy your chat...`",
                    )
        except Exception as e:
            return await edit_or_reply(event, f"**Gagal : **`{str(e)}`")
        await edit_or_reply(
            event,
            f"{_format.mentionuser(user.first_name ,user.id)} `telah diunmute di {event.chat.title}\n`",
        )
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
...
async def kick(usr):
    if usr.fwd_from:
        return
    if not usr.is_group:
        await edit_or_reply(usr, "`Maaf, ini bukan grup!!!`")
        return
...
async def pin(msg):
    ...
    if not to_pin:
        return await edit_delete(msg, "`Reply pesan yang akan dipin.`", 5)
...
async def pin(msg):
    ...
    if not to_unpin and options != "all":
        await edit_delete(msg, "`Reply ke pesan yang akan di unpin atau ketik .unpin all`", 5)
        return
    if to_unpin and not options:
        ...
        return await edit_delete(
            msg, "`Reply ke pesan yang akan di unpin atau ketik .unpin all`", 5
        )
...
async def _(event):
    ...
    if not event.is_group:
        await edit_or_reply(event, "`Maaf, ini bukan grup!!`")
        return
    ...
    if c.admin_rights or c.creator:
        ...
        await edit_or_reply(
            event, "`Maaf, memerlukan gelar admin untuk menjalankan perintah ini.`"
        )
        await sleep(3)
        try:
            await event.delete()
