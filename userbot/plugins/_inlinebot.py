...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("**Tokai-Ubot") and event.query.user_id == bot.uid:
            buttons = [
                (
                    custom.Button.inline("Stats", data="stats"),
                    Button.url("Repo", "https://github.com/Tokai-Robo/tokaiuserbot"),
                )
            ]
            ...
            result = builder.article(
                "© Userbot Help",
                text="{}\nJumlah plugin terdeteksi: {}".format(query, len(CMD_LIST)),
                buttons=buttons,
                link_preview=False,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                ...
                text=f"🔒 Pesan rahasia kepada {sandy}, hanya dia yang bisa membukanya.",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            reply_pop_up_alert = "Deploy botmu sendiri, ini bukan punyamu! "
        ...
        if os.path.exists("./userbot/secrets.txt"):
            ...
            try:
                ...
                if event.query.user_id in ids:
                    ...
                    reply_pop_up_alert = "deploy botmu sendiri, idiot!"
            except KeyError:
                reply_pop_up_alert = "Pesan ini tidak lagi exist pada server bot"
        else:
            reply_pop_up_alert = "Pesan ini tidak lagi exist "
        await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            reply_pop_up_alert += (
                "Pakai .unload {} untuk menghapus plugin ".format(plugin_name)
            )
            try:
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
            except BaseException:
